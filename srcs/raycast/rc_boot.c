...
/*   Updated: 2025/09/04 21:14:44 by brian            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int rc_boot_init(t_brain *b) {
  ...
  // Hooks (uncomment once you’re ready to drive from main)
  // mlx_loop_hook(b->ctx->mlx_ptr, rc_boot_loop, b);
  // mlx_hook(b->ctx->win_ptr, 17, 0, rc_boot_close, NULL);
  // mlx_hook(b->ctx->win_ptr, 2, 1L<<0, rc_boot_keydown, b);
  return 0;
}
